...
public class UserService : IUserService
{
    ...
    public List<User> GetAllStaffMembers()
    {
        return _pustokDbContext.Users
            .Where(u =>
                u.Role == Role.Values.Admin ||
                u.Role == Role.Values.Moderator ||
                u.Role == Role.Values.SuperAdmin)
            .ToList();
    }

    public List<User> GetAllCustomers()
    {
        var customers = _pustokDbContext.Users.Where(u => u.Role == Role.Values.User).ToList(); 
        return customers;   
    }
}
